...
namespace WindowsLayoutSnapshot
{
    internal static class UXFormat
    {
        ...
        public static string FormatPositiveDuration(TimeSpan ts)
        {
            if (ts < TimeSpan.Zero) ts = -ts;
            ...
            bool noMs = IsCloseToWholeNumber(sec);
            if (sec < 60) return noMs ? ts.ToString(@"s\ \s\e\c", NumberFormatInfo.InvariantInfo) : ts.ToString(@"s\.fff\ \s\e\c", NumberFormatInfo.InvariantInfo);
            if (sec < 60 * 60) return noMs ? ts.ToString(@"mm\:ss\ \m\i\n", NumberFormatInfo.InvariantInfo) : ts.ToString(@"mm\:ss\.fff\ \m\i\n", NumberFormatInfo.InvariantInfo);
            if (sec < 24 * 60 * 60) return noMs ? ts.ToString(@"hh\:mm\:ss\ \h", NumberFormatInfo.InvariantInfo) : ts.ToString(@"hh\:mm\:ss\.fff\ \h", NumberFormatInfo.InvariantInfo);

            return noMs
                ? ts.ToString(@"d \d\a\y\s hh\:mm\:ss\", NumberFormatInfo.InvariantInfo)
                : ts.ToString(@"d \d\a\y\s hh\:mm\:ss\.fff", NumberFormatInfo.InvariantInfo);
        }
